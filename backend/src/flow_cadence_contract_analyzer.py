...
class ImpactType(int, Enum):
    Low = 1
    Medium = 2
    High = 3

class FlowCadenceContractAnalyzer:

    def analyze_contract(self, contract_code):
        vulnerabilities = []
        findings_summary = {
            ImpactType.Low.name:0,
            ImpactType.Medium.name:0,
            ImpactType.High.name:0
        }

        # Split the code into lines
        contract_code = contract_code.replace('\r\n', '\n').replace('\r', '\n')  # Normalize line endings
        contract_code = contract_code.replace('%0A', '\n').replace('%0D', '\r')
        lines = contract_code.split('\n')
        is_function_public = False
        setter_variable = None  # Initialize the variable
        function_parameters_names = []
        assignment_variables = []
        for i, line in enumerate(lines, start=1):
            line = line.strip()  # remove leading/trailing whitespace
            if line.startswith('pub fun'):
                is_function_public = True
                if '(' in line and ')' in line and '{' in line:
                    # is_function_body = True
                    function_parameters = line.split('(')[1].split(')')[0].split(',')
                    function_parameters_names = []
                    for param in function_parameters:
                        param = param.strip().split(':')[0]
                        if param and len(param) > 0:
                            function_parameters_names.append(param)
            elif line.startswith('fun'):
                is_function_public = False
                issue = {
                    'impact': ImpactType.Low.name,
                    'line_number': i,
                    'line': line,
                    'description': 'Issue found: Function without access control'
                }
                vulnerabilities.append(issue)
                findings_summary[issue['impact']] += 1

            elif 'pub var' in line:  # check if the line declares a public variable
                issue = {
                    'impact': ImpactType.Medium.name,
                    'line_number': i,
                    'line': line,
                    'description': 'Issue found: Public variable that could be manipulated'
                }
                vulnerabilities.append(issue)
                findings_summary[issue['impact']] += 1
            elif '=' in line and not self.contains_conditional_operators(line):  # Ensure setter_variable is not None
                # print(line)
                left_side, right_side = line.split("=")
                left_side = left_side.strip()
                right_side = right_side.strip()

                for param in function_parameters_names:
                    if  param in right_side and is_function_public:
                        if 'let ' in left_side:
                            let, variable_name = line.split("let ")
                            variable_name = variable_name.strip()
                            if variable_name and len(variable_name) > 0:
                                function_parameters_names.append(variable_name)
                        elif 'self.' in left_side:
                            issue = {
                                'impact': ImpactType.High.name,
                                'line_number': i,
                                'line': line,
                                'description': 'Issue found: Public function that could change state'
                            }
                            vulnerabilities.append(issue)
                            findings_summary[issue['impact']] += 1
    ...
    def contains_conditional_operators(self, line):
        conditional_operator = ["==", ">=", "<=", ">", "<"]
        for operator in conditional_operator:
            if operator in line:
                return True
        return False

    def calculate_risk_score(self, findings):
        impact_scores = {"Low": ImpactType.Low.value, "Medium": ImpactType.Medium.value, "High": ImpactType.High.value}
        total_score = 0
        max_score = 0
        for issue in findings:
            impact_level = issue["impact"]
            total_score += (impact_scores[impact_level])
...
if __name__ == "__main__":
    ...
    """
    flow_cadence_contract_analyzer = FlowCadenceContractAnalyzer()
    findings = flow_cadence_contract_analyzer.analyze_contract(contract_code)
    # print(flow_cadence_contract_analyzer.calculate_risk_score(findings))
